...
function SegmentsAndLabel() {
    ...
    useEffect(() => {
        ...
        fetch('https://34.133.130.136:5000/api/images')
            ...
            .then(data => {
                ...
                const urls = filteredData.map(filename => `https://34.133.130.136:5000/uploads/${filename}`);
                setImageUrls(urls);
            })
            .catch(error => {
    ...
    const deleteImage = () => {
        ...
        fetch(`https://34.133.130.136:5000/api/delete/${filename}`, {
            method: 'DELETE',
        })
            .then(response => response.json())
            .then(data => {
    ...
    const handleEdit = async () => {
        if (newTitle.trim() !== '') {
            ...
            const response = await fetch(`https://34.133.130.136:5000/api/edit`, {
                method: 'POST',
                headers: {
