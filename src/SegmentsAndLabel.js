...
function SegmentsAndLabel() {
    ...
    useEffect(() => {
        ...
        fetch('http://34.133.130.136:5000/api/images')
            ...
            .then(data => {
                ...
                const urls = filteredData.map(filename => `http://34.133.130.136:5000/uploads/${filename}`);
                setImageUrls(urls);
            })
            .catch(error => {
    ...
    const deleteImage = () => {
        ...
        fetch(`http://34.133.130.136:5000/api/delete/${filename}`, {
            method: 'DELETE',
        })
            .then(response => response.json())
            .then(data => {
    ...
    const handleEdit = async () => {
        if (newTitle.trim() !== '') {
            ...
            const response = await fetch(`http://34.133.130.136:5000/api/edit`, {
                method: 'POST',
                headers: {
