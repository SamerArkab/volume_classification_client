...
function SegmentsAndLabel() {
    ...
    useEffect(() => {
        ...
        fetch('https://nut-values-server.chickenkiller.com/api/images')
            ...
            .then(data => {
                ...
                const urls = filteredData.map(filename => `https://nut-values-server.chickenkiller.com/uploads/${filename}`);
                setImageUrls(urls);
            })
            .catch(error => {
    ...
    const deleteImage = () => {
        ...
        fetch(`https://nut-values-server.chickenkiller.com/api/delete/${filename}`, {
            method: 'DELETE',
        })
            .then(response => response.json())
            .then(data => {
    ...
    const handleEdit = async () => {
        if (newTitle.trim() !== '') {
            ...
            const response = await fetch(`https://nut-values-server.chickenkiller.com/api/edit`, {
                method: 'POST',
                headers: {
